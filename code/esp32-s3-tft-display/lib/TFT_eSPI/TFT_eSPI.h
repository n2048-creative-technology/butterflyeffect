...
class TFT_eSPI : public Print { friend class TFT_eSprite; // Sprite class has access to protected members
 ...
 public:
  ...
  void     fillScreen(uint32_t color),
           fillScreen(uint32_t color, uint8_t randomizePercent),
  ...
  enum GlitchType {
    GLITCH_NONE = 0,
    GLITCH_BIT_RANDOM,     // Random bit flips
    GLITCH_CHANNEL_SWAP,   // Swap RGB channels
    GLITCH_SCANLINES,      // Add scanline effect
    GLITCH_ROW_SKIP        // Skip random rows
  };
  
  GlitchType _glitchType = GLITCH_NONE;
  uint8_t _glitchIntensity = 0; // 0-100
  
  // Randomization control for bit error effects (legacy)
  void setRandomizePercent(uint8_t percent) { Print::setRandomizePercent(percent); }
  uint8_t getRandomizePercent() const { return Print::getRandomizePercent(); }
  
  // Overloaded write method with randomization parameter
  size_t write(const uint8_t *buffer, size_t size, uint8_t randomizePercent) {
    return Print::write(buffer, size, randomizePercent);
  }
  
  // Glitch effect control
  void setGlitchType(GlitchType type);
  void setGlitchIntensity(uint8_t intensity);
  GlitchType getGlitchType() const { return _glitchType; }
  uint8_t getGlitchIntensity() const { return _glitchIntensity; }
  
  // Internal glitch application methods
  uint16_t scaleColor(uint16_t color, float factor);
  uint16_t applyGlitchToColor(uint16_t color);
  void fillScreenWithGlitch(uint16_t color, GlitchType type, uint8_t intensity);
 ...
 private:
  ...
  uint8_t  colstart = 0, rowstart = 0; // Screen display area to CGRAM area coordinate offsets
