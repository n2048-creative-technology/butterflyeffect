...
/***************************************************************************************
** Function name:           scaleColor
** Description:             Scale a 16-bit color by a factor (0.0-1.0)
***************************************************************************************/
uint16_t TFT_eSPI::scaleColor(uint16_t color, float factor) {
...
/***************************************************************************************
** Function name:           setGlitchType
** Description:             Set the glitch effect type
***************************************************************************************/
void TFT_eSPI::setGlitchType(GlitchType type) {
  _glitchType = type;
}

/***************************************************************************************
** Function name:           setGlitchIntensity
** Description:             Set the glitch effect intensity (0-100)
***************************************************************************************/
void TFT_eSPI::setGlitchIntensity(uint8_t intensity) {
  _glitchIntensity = (intensity > 100) ? 100 : intensity;
}

/***************************************************************************************
** Function name:           applyGlitchToColor
** Description:             Apply glitch effect to a single color value
***************************************************************************************/
uint16_t TFT_eSPI::applyGlitchToColor(uint16_t color) {
  if (_glitchType == GLITCH_NONE || _glitchIntensity == 0) {
    return color;
  }
  
  switch (_glitchType) {
    case GLITCH_BIT_RANDOM: {
      // Random bit flips (existing behavior via Print class)
      if (random(100) < _glitchIntensity) {
        uint16_t bitToFlip = random(16);
        color ^= (1 << bitToFlip);
      }
      break;
    }
    case GLITCH_CHANNEL_SWAP: {
      ...
      if (random(100) < _glitchIntensity) {
        // Randomly swap two channels
        uint8_t swapType = random(3);
        if (swapType == 0) {
          // Swap R and G
          uint16_t temp = r;
          r = g >> 1; // G is 6-bit, R is 5-bit
          g = temp << 1;
        } else if (swapType == 1) {
          // Swap G and B
          uint16_t temp = g;
          g = b << 1; // B is 5-bit, G is 6-bit
          b = temp >> 1;
        } else {
          // Swap R and B
          uint16_t temp = r;
    ...
    case GLITCH_SCANLINES: {
      // For scanlines, we don't modify the color here - it's applied per-row
      break;
    }
    case GLITCH_ROW_SKIP: {
      // For row skip, we don't modify the color here - it's applied per-row
      break;
    }
  }
  return color;
...
void TFT_eSPI::fillScreen(uint32_t color)
{
  ...
  if (_glitchType != GLITCH_NONE && _glitchIntensity > 0) {
    // Use the glitch-aware version
    uint16_t color16 = (uint16_t)(color >> 8 | color & 0xFF);
    fillScreenWithGlitch(color16, _glitchType, _glitchIntensity);
  } else if (getRandomizePercent() > 0) {
    // Legacy bit randomization
    fillScreen(color, getRandomizePercent());
  } else {
...
/***************************************************************************************
** Function name:           fillScreenWithGlitch
** Description:             Fill screen with glitch effect
***************************************************************************************/
void TFT_eSPI::fillScreenWithGlitch(uint16_t color, GlitchType type, uint8_t intensity)
{
  if (intensity == 0) {
    fillRect(0, 0, _width, _height, color);
    return;
  ...
  uint32_t rowSize = _width;
  uint16_t *rowBuffer = new uint16_t[rowSize];
  
  for (int32_t y = 0; y < _height; y++) {
    bool skipRow = false;
    uint16_t rowColorMod = 0; // Color modification for this row
    
    // Apply per-row effects
    switch (type) {
      case GLITCH_ROW_SKIP:
        ...
        break;
      case GLITCH_SCANLINES:
        // Add scanline effect - darken alternating rows
        if (y % 2 == 1) {
          // Darken odd rows by scaling down
          rowColorMod = scaleColor(color, 0.3);
        }
        break;
      default:
        break;
    ...
    if (skipRow) {
      // For row skip, fill with random noise
      for (uint32_t x = 0; x < rowSize; x++) {
        rowBuffer[x] = random(65536); // Random color
      }
    } else {
      // Fill row buffer with base color (possibly modified)
      uint16_t finalColor = (rowColorMod != 0) ? rowColorMod : color;
      for (uint32_t x = 0; x < rowSize; x++) {
        rowBuffer[x] = finalColor;
      }
      
      // Apply per-pixel effects
      switch (type) {
        case GLITCH_BIT_RANDOM: {
          size_t totalBits = rowSize * 16;
          size_t bitsToRandomize = (totalBits * intensity) / 100;
          for (uint32_t i = 0; i < rowSize && bitsToRandomize > 0; i++) {
            for (uint8_t bit = 0; bit < 16 && bitsToRandomize > 0; bit++) {
              if (random(100) < intensity) {
                rowBuffer[i] ^= (1 << bit);
                bitsToRandomize--;
              }
        ...
        case GLITCH_CHANNEL_SWAP: {
          for (uint32_t i = 0; i < rowSize; i++) {
            if (random(100) < intensity) {
              rowBuffer[i] = applyGlitchToColor(rowBuffer[i]);
            }
          }
          break;
        }
...
void TFT_eSPI::fillScreen(uint32_t color, uint8_t randomizePercent)
{
  ...
  uint16_t color16 = (uint16_t)(color >> 8 | color & 0xFF);
  fillScreenWithGlitch(color16, GLITCH_BIT_RANDOM, randomizePercent);
}
...
////////////////////////////////////////////////////////////////////////////////////////
#include "Extensions/Button.inc"

#include "Extensions/Sprite.inc"

#ifdef SMOOTH_FONT
  #include "Extensions/Smooth_font.inc"
#endif
